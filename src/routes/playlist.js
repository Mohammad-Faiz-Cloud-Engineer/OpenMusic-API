...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    const result = await scraper.getPlaylist(id.trim());
    metadataCache.set(cacheKey, result);
    res.json(result);
  } catch (err) {
    if (err.message?.includes('not found')) {
      return res.status(404).json({ error: 'playlist_not_found', source, message: err.message });
    }
    ...
    console.error(`[playlist] ${source} error:`, err.message);
    res.status(500).json({ error: 'playlist_failed', source, message: err.message });
  }
});
