...
def load_tally_data():
    """Load, normalise, and clean tally data. Thread-safe."""
    with _tally_lock:
        return _load_tally_data_unsafe()


def _load_tally_data_unsafe():
    """Must be called with _tally_lock held."""
    ensure_data_dir()
    path = get_tally_path()
    if not os.path.exists(path):
        data = default_tally_data()
        _save_tally_data_unsafe(data)
        return data
...
def save_tally_data(data):
    """Persist tally data atomically. Thread-safe."""
    with _tally_lock:
        _save_tally_data_unsafe(data)


def _save_tally_data_unsafe(data):
    """Must be called with _tally_lock held."""
    ensure_data_dir()
    path = get_tally_path()
...
def load_and_save_tally(transform):
    """
    Atomic read-modify-write helper.

    Accepts a callable that receives the current tally dict and returns the
    (possibly mutated) dict to persist.  The entire operation runs under the
    lock so callers never need to acquire it themselves.
    """
    with _tally_lock:
        data = _load_tally_data_unsafe()
        result = transform(data)
        _save_tally_data_unsafe(result)
        return result
