...
# Protects the in-process catalog cache and all file writes so concurrent
# requests cannot read a partially-written catalog or corrupt the JSON file.
...
def _read_catalog():
    """Return a fresh copy of the catalog, using the mtime-based cache."""
...
def _write_catalog(catalog):
    """Write catalog atomically and update the in-process cache."""
    global _catalog_cache, _catalog_cache_mtime
