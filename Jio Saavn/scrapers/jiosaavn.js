...
const { upsertSongRecords } = require('../recommendation');
...
async function search(query) {
  ...
  // Feed results into the recommendation catalog asynchronously
  if (result.results?.length) upsertSongRecords(result.results);
  return result;
}
...
async function getAlbum(id) {
  ...
  if (result.tracks?.length) upsertSongRecords(result.tracks);
  return result;
}
